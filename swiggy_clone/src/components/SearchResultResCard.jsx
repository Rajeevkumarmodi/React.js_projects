  ...
  return (
    <div className=" bg-white py-5 cursor-pointer">
      ...
      <div className="flex pl-4 flex-col justify-center min-h-[108px]">
        <p className="text-[#3e4152] font-semibold text-ellipsis overflow-hidden whitespace-nowrap">
          {name}
        </p>
        <p className="flex text-sm text-[#696b79] font-medium mt-[6px]">
          <span>⭐ {avgRating}</span>{" "}
          <span className="pl-2">{sla.slaString}</span>{" "}
          <span className="pl-2">{costForTwoMessage}</span>
        </p>
        <p className="mt-1 text-sm line-clamp-1 text-[#93959f]">
          {cuisines.join(", ")}
        </p>
      </div>
      {/* </Link> */}
